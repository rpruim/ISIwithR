...
@
%how to simulate difference in pairs

We simulate a world in which $\mu_d = 0$:
<<Figure7.6, cache=TRUE, tidy=FALSE>>=

@
%1000 rep of swapped difference in pairs
<<Figure7.7>>=
...
\begin{enumerate}
  \item
    ...
    Test statistic:  $\bar x_d = $ (the mean difference in sample)
  \item
    We simulate a world in which $\mu_d = 0$:
...
<<Figure7.9>>=
dotPlot(~(Small - Large), data = BowlsMMs, nint = 100, ylim = c(0.75, 1.25),cex = .05)
@
