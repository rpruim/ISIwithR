...
<<Figure7.3>>=
dotPlot(~ narrow, data = FirstBase, nint = 40, xlim = c(4.50, 6.50))
dotPlot(~ wide, data = FirstBase, nint = 40, xlim = c(4.50, 6.50))
...
<<Figure7.8>>=
require(tidyr)
FirstBase2 <- FirstBase %>% gather(angle, time)
head(FirstBase2)
FirstBase.Null <- do(1000) * diffmean( time ~ shuffle(angle), data=FirstBase2 )
dotPlot( ~diffmean, data=FirstBase.Null, nint=50 )
@
