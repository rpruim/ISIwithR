...
simulation.cancer2 <- do(1000) * rflip(33, 0.70)
...
@
%typo in book? left limit is 0.4545 not 0.491
\end{enumerate}
...
simulation.cancer3 <- do(1000) * rflip(33, 0.80)
...
@
%typo in book? left limit is 0.6667 not 0.691
\end{enumerate}
...
@
% Table 3.2 not included

% Confidence Level

% Table 3.3 not included
...
simulation.kiss2 <- do(1000) * rflip(124, 0.6)
...
An easy way to find a confidence interval in \R\ is to use \function{prop.test()} or \function{binom.test()} which by default calculates a 95\% confidencen interval in its results.
...
dotPlot(~ prop, data = simulation.amer, groups = (prop <= 0.750 | prop >= 0.80), width = 0.001)
@
<<>>=
favstats(~ prop, data = simulation.amer)
prop(~ (prop <= 0.750 | prop >= 0.80), data = simulation.amer)
@
  \item
    Approximate test for proportions:
<<>>=
...
dotPlot(~ prop, data = simulation.amer2, groups = (prop <= 0.20 | prop >= 0.80), width = 0.001)
@
<<>>=
favstats(~ prop, data = simulation.amer2)
prop(~ (prop <= 0.20 | prop >= 0.80), data = simulation.amer2)
@
  \item
    Approximate test for proportions:
<<>>=
...
<<>>=
sd <- sd(~ prop, data = simulation.amer); sd
z <- (0.80 - 0.775) / sd; z
...
simulation.CI <- CIsim(100, samples = 100, rdist = rbinom, args = list(size = 1, prob = 0.5), method = binom.test, method.args = list(success = 1), verbose = FALSE, estimand = 0.5)
@
...
<<message=FALSE,opts.label="fig1">>=
require(Hmisc)
xYplot(Cbind(estimate, lower, upper) ~ sample, data = simulation.CI, par.settings = col.mosaic(), groups = cover)
@

Simulate 1000 sample proportions and calculate the 95\% confidence intervals:
<<cache=TRUE>>=
simulation.CI2 <- CIsim(100, samples = 1000, rdist = rbinom, args = list(size = 1, prob = 0.5), method = binom.test, method.args = list(success = 1), verbose = FALSE, estimand = 0.5)
@
...
<<cache=TRUE>>=
simulation.CI3 <- CIsim(100, samples = 1000, rdist = rbinom, args = list(size = 1, prob = 0.5), conf.level = 0.90, method = binom.test, method.args = list(success = 1), verbose = FALSE, estimand = 0.5)
@
...
<<cache=TRUE>>=
simulation.CI4 <- CIsim(400, samples = 100, rdist = rbinom, args = list(size = 1, prob = 0.5), conf.level = 0.90, method = binom.test, method.args = list(success = 1), verbose = FALSE, estimand = 0.5)
@
