...
Further, we can repeat each simulation many times by multiplying it by \function{do()}. When using \function{do}, you should assign the simulation a name by using an arrow (\verb!<-!) so that you are creating a new data set with all of the repetitions. In this case, we are naming the simulation \dataframe{game.}
<<cache=TRUE>>=
