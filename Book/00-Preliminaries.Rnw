...
In the \tab{Packages} tab, check the boxes next to the following package to
load them:
\begin{itemize}
...
@
You can't really see the dots in this plot the defaults for \function{dotPlot()} may not be the best way to examine this particular data set.
...
dotPlot(~ Time, data = OldFaithful1, width = 1, cex = .25)
dotPlot(~ Time, data = OldFaithful1, width = 1, cex = .15)
dotPlot(~ Time, data = OldFaithful1, width = 1, cex = .15, xlab = "time until next eruption (min)")
@
...
dotPlot(~ TimeBetween, groups = EruptionType, data = OldFaithful2, width = 1, cex = 0.15)
@
...
dotPlot(~ TimeBetween | EruptionType, data = OldFaithful2, layout = c(1,2), width = 1, cex = 0.15)
@
...
Further, we can repeat each simulation many times by multiplying it by \function{do()}. When using \function{do}, you should assign the simulation a name by using an arrow ``\<-" so that you are creating a new data set with all of the repetitions. In this case, we are naming the simulation \dataframe{game.}
<<cache=TRUE>>=
...
<<>>=
dotPlot(~ prop, data = game, width = 0.1)
@
