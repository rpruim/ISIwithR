...
@
  \item
    ...
    For the \term{p-value}, you can use the \function{prop()} function and input \option{(prop <= 1/6)} to find the proportion of samples that is less than or equal to the observed proportion in the data set \data{sim.sci}.
<<Figure1.8>>=
