...
head(sim2, 3)
dotPlot(~ heads, data = sim2, width = 1, cex = 3, groups = (heads <= 16 & heads >= 16))
@
Notice the way we defined \argument{groups} as \option{(heads <= 16 \& heads >= 16)} in order to differentiate the observations where \variable{heads} equals 16.
...
head(sim.harley2, 3)
dotPlot(~ heads, data = sim.harley2, width = 1, cex = 3, groups = (heads <= 9 & heads >= 9))
@

\subsubsection*{Another Study}

<<Exploration1.1.23>>=
dotPlot(~ heads, data = sim.harley2, width = 1, cex = 3, groups = (heads <= 6 & heads >= 6))
@
...
@
  \item
    ...
    For the \term{p-value}, you can use the \function{prop()} function and input \option{(prop <= 1/6)} to find the proportion of samples that is less than or equal to the observed proportion in the data set \dataframe{sim.sci}.
<<Figure1.8>>=
...
Now that we've covered normal approximation, we can examine the rest of the output from \function {xpnorm()}. Because it's a two-sided test, we can input both the observed statistic (148/283) and the statistic that is as extreme as the observed (135/283). 
