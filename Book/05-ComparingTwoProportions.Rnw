...
sim.yawn <- 
  do(1000) * diffprop(shuffle(response) ~ yawnSeed, level = "Yawn", quiet = TRUE, 
                      data = Yawning)
head(sim.yawn, 3)
